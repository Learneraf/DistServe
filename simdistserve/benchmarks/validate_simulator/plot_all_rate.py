...
def parse_comparison_txt(filepath):
    ...
    返回:
        ...
        指标名称如 'TTFT SLO met rate', 'TPOT SLO met rate' 等
    """
    metric_aliases = [
        ('TTFT SLO met rate', ['TTFT SLO met rate', 'Prefill SLO met rate']),
        ('TPOT SLO met rate', ['TPOT SLO met rate', 'Decode SLO met rate']),
        ('Total SLO met rate', ['Total SLO met rate']),
        ('Both (TTFT+TPOT) SLO met rate', [
            'Both (TTFT+TPOT) SLO met rate',
            'Both (Prefill+Decode) SLO met rate',
        ]),
    ]
    target_metrics = [alias for _, aliases in metric_aliases for alias in aliases]
            ...
            if len(percents) >= 2:
                ...
                for canonical, aliases in metric_aliases:
                    if any(alias in line for alias in aliases):
                        results[canonical] = (exp_percent, csv_percent)
                        break
    return results
...
def plot_model_subplots(model_name, data, output_path):
    ...
    # 指标映射：原始名称 -> 子图显示名称
    metrics_info = {
        'TTFT SLO met rate': ('TTFT SLO Met Rate', 'TTFT'),
        'TPOT SLO met rate': ('TPOT SLO Met Rate', 'TPOT'),
        'Total SLO met rate': ('Total SLO Met Rate', 'Total'),
        'Both (TTFT+TPOT) SLO met rate': ('Both SLO Met Rate', 'Both')
    }
