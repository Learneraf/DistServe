...
METRICS = [
    ("ttft", "TTFT SLO met rate", "TTFT"),
    ("tpot", "TPOT SLO met rate", "TPOT"),
    ("total", "Total SLO met rate", "Total"),
    ("both", "Both (TTFT+TPOT) SLO met rate", "Both"),
]
METRIC_BY_LABEL = {label: (key, short) for key, label, short in METRICS}
METRIC_BY_LABEL.update(
    {
        "Prefill SLO met rate": ("ttft", "TTFT"),
        "Decode SLO met rate": ("tpot", "TPOT"),
        "Both (Prefill+Decode) SLO met rate": ("both", "Both"),
    }
)
PATH_RE = re.compile(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--slo-root",
        type=Path,
        default=Path("./result/slo"),
    )
    parser.add_argument("--backends", nargs="+", default=["cuda_distserve", "ascend_vllm"])
